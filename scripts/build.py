...
(function() {{
  ...
  // match_fn receives the repo object; return true to include in this category.
  // Repos matched earlier categories are excluded from later ones.
  const CATEGORIES = [
    ['Formal Methods & Verification', r =>
      /tla|formal|coq|rocq|alloy|resilient|goldbach|verification|fm-cb|TLAJVM/i.test(r.name) ||
      (r.topics||[]).some(t => /tla|formal|coq/i.test(t))
    ],
    ['AI, LLMs & Machine Learning', r =>
      /llm|gpt|ollama|ai-|picai|sign-language|rvc|tts|handwriting|als-sig|reelforge|rl-agent|connect-4|fortnite|oneshot|audio|yeat/i.test(r.name) ||
      (r.topics||[]).some(t => /llm|ai|ml|gpt/i.test(t))
    ],
    ['macOS, iOS & Desktop', r =>
      /mac|ios|swift|tunes2tube|dexcom|chess-?stats|youtube-dl|soundboard|apple-music|library-?room|tdx|dexval|t-square/i.test(r.name)
    ],
    ['Systems, Languages & Tools', r =>
      /udp|itch|xoroshiro|parking|cube|palindrome|grader|roman|etl|fg-|email-?extract|scala|copilot-cli|gitkey|flatten|notify-agent|reverse-/i.test(r.name)
    ],
    ['Web & Front-End', r =>
      /github[.]io$|web|front|site|pitch|fb-clone|design-skill|ai4fm|dev[.]|caterpillar|gcf|margaux|daily|bio-ops|sneaker|slot|spa-|cone-|uzz/i.test(r.name)
    ],
    ['Hackathons & Coursework', r =>
      /hack|comp[0-9]|cs50|csapp|health|aoc|march-mad|movie-?rec|blackjack|anagram|chat(?!tla)|serenity/i.test(r.name)
    ],
  ...
  async function render() {{
    ...
    try {{
      const all = (await Promise.all(SOURCES.map(fetchAll))).flat();
      const repos = all.filter(r => !SKIP.has(r.name));
